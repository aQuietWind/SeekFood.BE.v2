...
public class PhoneCaffeine extends JvmCaffeineParent<Long,String> {
    ...
    @Autowired
    public PhoneCaffeine(MerchantCaffeineConfig merchantCaffeineConfig) {
    ...
    public void init() {
        super.CACHE = Caffeine.newBuilder()
                .maximumSize(merchantCaffeineConfig.getPhoneNumber().getMaxSize())
                .expireAfterWrite(merchantCaffeineConfig.getPhoneNumber().getExpireTime(), TimeUnit.MINUTES)
                .recordStats()
                .build();
    }
