...
@Component
public class FundCaffeine extends JvmCaffeineParent<Long, FundDTO> {
    // 构造注入配置
    private final FundCaffeineConfig fundCaffeineConfig;
    @Autowired
    public FundCaffeine(FundCaffeineConfig fundCaffeineConfig) {
    ...
    public void init() {
        super.CACHE = Caffeine.newBuilder()
                .maximumSize(fundCaffeineConfig.getFund().getMaxSize())
                .expireAfterWrite(fundCaffeineConfig.getFund().getExpireTime(), TimeUnit.MINUTES)
                .recordStats()
                .build();
    }
