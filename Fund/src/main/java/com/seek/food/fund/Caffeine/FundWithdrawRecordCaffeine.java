...
@Component
public class FundWithdrawRecordCaffeine extends JvmCaffeineParent<Long, FundWithdrawRecordDTO> {
    // 构造注入配置
    private final FundCaffeineConfig fundCaffeineConfig;
    @Autowired
    public FundWithdrawRecordCaffeine(FundCaffeineConfig fundCaffeineConfig) {
    ...
    public void init() {
        super.CACHE = Caffeine.newBuilder()
                .maximumSize(fundCaffeineConfig.getFundWithdrawRecord().getMaxSize())
                .expireAfterWrite(fundCaffeineConfig.getFundWithdrawRecord().getExpireTime(), TimeUnit.MINUTES)
                .recordStats()
                .build();
    }
